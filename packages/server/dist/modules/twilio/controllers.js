...
function _getPhoneNumbers() {
  _getPhoneNumbers = _asyncToGenerator(function* (req, res) {
    ...
    try {
      phoneNumbers = yield client.availablePhoneNumbers("US").local.list({
        areaCode: areaCode
      });
    } catch (error) {
...
function _createUserTwilio() {
  _createUserTwilio = _asyncToGenerator(function* (req, res) {
    ...
    try {
      existingUser = yield _model.default.findById(userID);
      twilioUser = yield client.api.accounts.create({
        friendlyName: existingUser.fullName
      });
      exisitingUser.sid = twilioUser.sid;
      yield exisitingUser.save();
      console.log("\n", exisitingUser);
      return res.status(200).send({
        updatedUser: exisitingUser
      });
    } catch (error) {
      return res.status(400).send({
        error: error
